...
class VigorDevice():
    ...
    def get_supply_humidity(self):
        """ 
        Gets the supply airflow humidity in %
        """
        command = 4037
        rr = self.client.read_input_registers(command, 1, unit=self.UNIT)
        if rr.isError():
            return self._handle_error_int(rr, "get_supply_humidity", command)

        result = rr.registers[0]
        _log.debug("get_supply_humidity: " + str(result) + " %")
        return result

    def get_extract_humidity(self):
        """ 
        Gets the extract airflow humidity in %
        """
        command = 4047
        rr = self.client.read_input_registers(command, 1, unit=self.UNIT)
        if rr.isError():
            return self._handle_error_int(rr, "get_extract_humidity", command)
